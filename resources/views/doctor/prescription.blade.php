...
<main class="bg-dark">
            ...
            <div class="container">
                <!-- <div class="d-flex justify-content-between mb-5">
                    <div class="mb-3 test1">
                        <h5 class="docname">{{"Dr. ".$doctor_info->fname." ".$doctor_info->lname}}</h5>
                        <ul class="list-unstyled">
                            <li>
                                <p class="deg2">{{$doctor_info->doctor->professional_degree}}</p>
                            </li>
                            <li>
                                <p class="deg2">{{$doctor_info->doctor->speciality}}</p>
                            </li>
                            <li>
                                <p class="deg2">{{$doctor_info->doctor->designation}}</p>
                            </li>
                            <li>
                                <p class="bmdcnum">BMDC no: {{$doctor_info->doctor->BMDC}}</p>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="mb-3 test1">
                        ...
                        @else
                        <img src="{{asset ('public/uploads/doctor/Barcode/'.$doctor_info->doctor->bar_image)}}" alt="Qr" class="img-fluid" width="100" height="100">
                        @endif
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-borderless">
                        <tr class=" ">
                            ...
                            <td class=""><span class="title">Today Date :{{ date('d/m/Y') }}</span> <span class="value pres_pt_date"></span></td>
                        </tr>
                        <tr class="">
                            ...
                            <td class=" "><span class="title">Next Appointment :
                                    @if($appointment != null && $appointment->date == \Carbon\Carbon::today())
                        ...
                        <div class="left">
                            ...
                            <div class="single MedHistory">
                                ...
                                <div class="complain">
                                    ...
                                    <div class="text-start text-capitalize"><span class="fw-bolder">BP</span> : {{$patient->bp_high}} / {{$patient->bp_low}}</div>
                                    <div class="text-start text-capitalize"><span class="fw-bolder">Heart Disease</span> : {{$patient->Heart_Disease}}</div>
                                    <div class="text-start text-capitalize"><span class="fw-bolder">Diabetic</span> : {{$patient->Diabetic}}</div>
                                    <div class="text-start text-capitalize"><span class="fw-bolder">Helpatics</span> : {{$patient->Helpatics}}</div>
                                    <div class="text-start text-capitalize"><span class="fw-bolder">Bleeding disorder</span> : {{$patient->Bleeding_disorder}}</div>
                                    <div class="text-start text-capitalize"><span class="fw-bolder">Allergy</span> : {{$patient->Allergy}}</div>

                                    @if($patient->gender == "Male")
                                    <div class="text-start"> </div>
                                    @else
                                    <div class="text-start text-capitalize"><span class="fw-bolder">Pregnant/Lactating</span> : {{$patient->Pregnant}}</div>
                                    @endif
                                    <div class="text-start text-capitalize"><span class="fw-bolder">Other</span> : {{$patient->other}}</div>
                                        ...
                                        <li>
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <div class="align-self-center">
                                                    ...
                                                    <button class="btn crud-btns delete-drug" data-drug-name="{{$drug->drug_name}}" value="{{$drug->id}}">
    ...
    <div>
        {{-- <button class="btn btn-primary print-btn d-print-none" onclick="printpage()" id="printpagebutton">--}}
        ...
        <button type="button" class="btn btn-primary back-btn px-2 submitPrescription d-print-none" value="{{$patient->id}}">
            <i class="fa-solid fa-arrow-left-long"></i>
            Back
        </button>
    </div>
    ...
    <div class="rightSideButton">
        ...
        <div class="mt-1 mb-2">
            @if($temp_drugs->count() != 0)
            <p class="rightSideHead d-print-none">Drug</p>
            @endif
            <div class="d-flex flex-column align-items-end tempDrug_list_btn">
                @foreach($temp_drugs as $temp_drug)
                <button type="button" class="btn btn-primary mb-1 templateBtn tempDrug" data-d_id="{{$doctor_info->id}}" data-p_id="{{$patient->id}}" value="{{$temp_drug->id}}">
                    {{ $temp_drug->title}}
                </button>
                @endforeach
            </div>
        ...
        <div class="mt-1 mb-2">
            @if($temp_advices->count() != 0)
            <p class=" rightSideHead d-print-none">Advice</p>
            @endif
            <div class="d-flex flex-column align-items-end advice_list_btn">
                @foreach($temp_advices as $temp_advice)
                <button type="button" class="btn btn-primary mb-1 templateBtn tempAdvice" value="{{$temp_advice->id}}">
                    {{ $temp_advice->title}}
                </button>
                @endforeach
            </div>
                ...
                <div class="tab-content" id="myTabContent">
                    <div class="tab-pane fade show active" id="advice_temp" role="tabpanel" aria-labelledby="advice_temp-tab">
                        <div class="d-flex justify-content-between align-items-center my-3 reloadCount">
                            ...
                            $remain = 5 - $count;
                            @endphp
                            <h6>Advice Template List @if($remain != 0) (Remain:{{$remain}}) @endif</h6>
                            @if($remain != 0)
                        ...
                        <table class="table table-bordered text-center align-middle advice_list">
                            ...
                            <tbody>
                                @forelse($temp_advices as $key=>$temp_advice)
                                <tr>
                                    <th scope="row">{{$key + 1}}</th>
                                    <td>{{$temp_advice->title}}</td>
                                    <td>@nl2br($temp_advice->description)</td>
                                    <td>
                                        <button type="button" class="btn crud-btns editTempAdvice advice{{$temp_advice->id}}" value="{{$temp_advice->id}}">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <div class="tab-pane fade" id="drug_temp" role="tabpanel" aria-labelledby="drug_temp-tab">
                        <div class="d-flex justify-content-between align-items-center my-3 reloadCount1">
                            ...
                            $remain1 = 3 - $count1;
                            @endphp
                            <h6>Drug Template List @if($remain1 != 0) (Remain:{{$remain1}}) @endif</h6>
                            @if($remain1 != 0)
                        ...
                        <table class="table table-bordered text-center align-middle drugTemp_list">
                            ...
                            <tbody>
                                @forelse($temp_drugs as $key=>$temp_drug)
                                <tr>
                                    <th scope="row">{{$key + 1}}</th>
                                    <td>{{$temp_drug->title}}</td>
                                    <td class="d-flex justify-content-center">
                                        <ul class="mb-0">
                                            @foreach($temp_drug->temp_drugs as $drug_info)
                                            <li><span class="fw-bolder">{{$drug_info->tempDrug_name.","}}</span>{{" [ ".$drug_info->tempDrug_time.", ".$drug_info->tempDrug_mealTime.", ".$drug_info->tempDrug_duration." day(s) ]"}}</li>
                                            @endforeach
                                        </ul>
                                    </td>
                                    <td>
                                        <button type="button" class="btn crud-btns deleteTempDrug" data-temp-drug="{{$temp_drug->title}}" value="{{$temp_drug->id}}">
                                            <i class="fa-solid fa-trash"></i>
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <div class="tab-pane fade" id="personal_info" role="tabpanel" aria-labelledby="personal_info-tab">
                        ...
                        <form action="{{route('updateDocFrom_prescription')}}" method="post" id="updateDocFrom_pres">
                            @csrf
                            <input type="hidden" name="doc_id" value="{{$doctor_info->doctor->id}}">
                            <div class="border rounded p-2">
                                <div class="mb-3">
                                    <label for="specialty" class="form-label">Specialty</label>
                                    <input type="text" class="form-control" id="specialty" name="specialty" placeholder="Enter Specialty" value="{{$doctor_info->doctor->speciality}}">
                                    <span class="text-danger test specialty_error"></span>
                                </div>
                                <div class="mb-3">
                                    <label for="post_grad_deg" class="form-label">Post Graduation Degrees</label>
                                    <input type="text" class="form-control" id="post_grad_deg" name="post_grad_deg" placeholder="Enter Post Graduation Degrees Like BDS (DU). MPH (NSU) FCPS (DDC)" value="{{$doctor_info->doctor->professional_degree}}">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="cur_desig_ins" name="cur_desig_ins" placeholder="Enter Current Designation And Institute Like Lecturer, Dhaka Dental College." value="{{$doctor_info->doctor->designation}}">
...
<div class="modal fade advice_mo" id="AdviceTemp_Add" tabindex="-1" aria-labelledby="AdviceTemp_AddLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form action="{{route('add_advice_template',$doctor_info->id)}}" method="POST" id="add_advice_temp">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
...
<div class="modal fade adviceEdit_mo" id="AdviceTemp_Edit" tabindex="-1" aria-labelledby="AdviceTemp_AddLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form action="{{route('update_advice_template',$doctor_info->id)}}" method="POST" id="update_advice_temp">
                @csrf
...
<div class="modal fade drugTemp_mo" id="DrugTemp_Add" tabindex="-1" aria-labelledby="DrugTemp_AddLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form action="{{route('add_drug_template',$doctor_info->id)}}" method="post" id="add_drug_temp">
                @csrf
                <!-- Modal Body -->
                <div class="modal-body">
...
<div class="modal fade " id="del-drug-temp" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <!-- Modal Body -->
            <div class="modal-body">
                <form action="{{route('delete_drug_template')}}" method="POST" id="DeleteDrugTemp">
                    @csrf
...
<div class="modal fade drug-mo" id="drug">
    <div class="modal-dialog modal-sm modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <input type="hidden" id="drug_d_id" value="{{$doctor_info->id}}">
            <input type="hidden" id="drug_p_id" value="{{$patient->id}}">
...
<div class="modal fade" id="editDrug">
    <div class="modal-dialog modal-sm modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <form action="{{route('update_drug')}}" method="POST" id="UpdateDrugForm">
                @csrf
                @method('PUT')
                <div class="modal-body">
    ...
    <div class="modal-dialog modal-md modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form action="{{route('prescription_add',[$doctor_info->id])}}" method="post">
                ...
                <!-- Modal Footer -->
                <div class="modal-footer">
                    <a href="{{route('view_patient',[$doctor_info->id,$patient->id])}}" class="btn btn-dark btn-sm rounded">No,Back</a>
...
<div class="modal fade " id="Medicine_Add" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content new-medicine-modal">
            ...
            <!-- Modal Body -->
            <div class="modal-body">
                <form action="{{route('medicine_add')}}" method="POST" id="AddMedicineForm">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3 drug-name">
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.editTempAdvice', function() {
            ...
            var url = "{{route('edit_advice_template',[':temp_advice_id'])}}";
        ...
        $(document).on('click', '.tempAdvice', function() {
            ...
            // alert(id);
            var url = "{{route('advice_form_template',[':temp_advice_id'])}}";
            url = url.replace(':temp_advice_id', temp_advice_id);
            // alert(drug_id);
            $.ajax({
                type: "GET",
        ...
        $(document).on('click', '.tempDrug', function() {
            ...
            // $(this).prop("disabled", true);
            // alert(id);
            var url = "{{route('drug_form_template',[':d_id',':p_id',':temp_drug_id'])}}";
        ...
        $(document).on('submit', '#DrugForm', function(e) {
            ...
            var url = "{{route('add_drug',[':d_id',':p_id'])}}";
            url = url.replace(':d_id', d_id);
            url = url.replace(':p_id', p_id);
            // alert(url);
        ...
        $(document).on('click', '.edit_drug', function() {
            ...
            var url = "{{route('edit_drug',[':drug_id'])}}";
        ...
        $(document).on('click', '.Medicine_editbtn', function() {
            ...
            // alert(medicine_id);
            let url = "{{route('edit_medicine',[':medicine_id'])}}";
        ...
        $(document).on('click', '.submitPrescription', function() {
            ...
            let url = "{{route('get_drug_info',[':p_id'])}}";
